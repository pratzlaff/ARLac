...
;ECLIPSE_GRIDPAR.PRO
;
; define a grid of parameters and make X-ray eclipse light curves
; for each grid point.
;
; a glorified wrapper for MK_XRAY_LC.PRO
;
; the grid is 4D:
; 	h1 (scale height of star 1)
; 	h2 (scale height of star 2)
; 	b2/b1 (intensity of star 2 relative to star 1)
;	cone (opening angle of cone, same for both stars)
...
; and changed to use SPHROJ/PATHPROJ rather than SPHERVOL (VK; MIM.XI).PRO
;
; ----------------------------------------------------------------
;-
...
if not keyword_set(cc) then cc=0		;opening angle of cone of inclusion
...
if n_elements(visual) eq 0 then visual=5	;display or not
window,0 & window,2 & wset,0 & loadct,38
...
if not keyword_set(occ) then occ=cc*2
...
;	define the grid for output
grid_h1=findgen(21)*0.1+0.1
grid_h2=grid_h1
grid_b21=findgen(9)*0.1+0.1 & grid_b21=[grid_b21,1.,1./reverse(grid_b21)]
grid_cone=[15.,30.,45.,60.,75.,90.,-75.,-60.,-45.,-30.,-15.]
ng1=n_elements(grid_h1)
ng2=n_elements(grid_h2)
ng3=n_elements(grid_b21)
ng4=n_elements(grid_cone)

;	define the outputs
all_lc=fltarr(nt,ng1,ng2,ng3,ng4)

if not keyword_set(i40) then i40=0L
for i4=i40,ng4-1L do begin	;{vary CONE
cc=grid_cone[i4]

if not keyword_set(i10) then i10=0L
for i1=i10,ng1-1L do begin	;{vary H1
  h1=grid_h1[i1]

  if not keyword_set(i20) then i20=0L
  for i2=i20,ng2-1L do begin	;{vary H2
    h2=grid_h2[i2]

    if not keyword_set(i30) then i30=0L
    for i3=i30,ng3-1L do begin	;{vary B2/B1
      b21=grid_b21[i3] & b2=b1*b21

      print,i1,i2,i3,i4

      ; get X-ray "image" by assuming that emissivity is constant/volume
      xra=[-(3.*a + r1+r2)*1.2/2.,(3.*a + r1+r2)*1.2/2.] & yra=xra
      ; the following two are time consuming, so don't repeat unless all
      ; variables have changed
      if or1 ne r1 or oh1 ne h1 or om1 ne m1 or om2 ne m2 or occ ne cc or $
       ofunct ne rfunct then img1=$
       sphroj(r1,r1+5*h1,verbose=visual,xrange=xra,yrange=yra,nx=nx,ny=ny,$
	 sclht=h1,rfunct=rfunct,coneang=cc)
      tvscl,img1
      if or2 ne r2 or oh2 ne h2 or om1 ne m1 or om2 ne m2 or occ ne cc or $
       ofunct ne rfunct then img2=$
       sphroj(r2,r2+5*h2,verbose=visual,xrange=xra,yrange=yra,nx=nx,ny=ny,$
	 sclht=h2,rfunct=rfunct,coneang=cc)
      tvscl,img2
      print,total(img1),total(img2)
      ;renormalize
      ;img1=img1*b1/total(img1)
      ;img2=img2*b2/total(img2)
      img1=img1/total(img1)
      img2=img2/total(img2)
      x0=0. & y0=0.
	...
	;print,i,lc[i]
	  ...
	  if visual gt 1 then tvscl,front+back else kilroy,dot=strtrim(i,2)+'.'
	endif
      ...
      lc=lc/max(lc)
      wset,2 & plot,[tgrid,1.+tgrid],[lc,lc] & wset,0
      help,h1,h2,b1,b2,cc
      all_lc[*,i1,i2,i3,i4]=lc[*]

      ; update old values
      om1=m1 & om2=m2 & or1=r1 & or2=r2 & oh1=h1 & oh2=h2 & occ=cc
      onx=nx & ony=ny & ofunct=rfunct

    endfor				;I3=I30,NG3-1}
    save,file='eclipse_gridpar4.save',/compress
  endfor			;I2=I20,NG2-1}
endfor			;I1=I10,NG1-1}
endfor		;I4=I40,NG4-1}
