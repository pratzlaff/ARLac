pro tprint,table,ascii,columns,row1,row2,noprint=noprint,pgbreak=pgbreak
;+
; NAME:
;   TPRINT 
; PURPOSE:
;   Routine to print an stsdas table to the terminal
; CALLING SEQUENCE:
;	tprint,table,ascii,columns,row1,row2,/noprint,/pgbreak
; INPUTS:
;	table - name of table to be printed
; OUTPUTS:
;	ascii - a string vector containing the formatted output
; OPTIONAL INPUTS:
...
; KEYWORDS:
;	noprint - if set, does not print out the table to the terminal
;		(useful when calling from other routines to extract info)
; HISTORY:
;	modified from tab_print (version 1, D. Lindler  Apr 89)
;	June 92 Vinay Kashyap
;-
;-----------------------------------------------------------------------------
;
; read in table
...
;
	...
	ascii = strarr(row2-row1+2)
	ascii(0) = line1
;
; loop on rows and print headers every 20 lines
;
	nlines=20
	for row=row1,row2 do begin

		if nlines ge 20 then begin
		...
		ascii(row+1) = line
		nlines=nlines+1
		if not keyword_set(pgbreak) then nlines = 1
	endfor
