/************************************************************************
   ...
   USAGE
	In IDL use this as:
	so = '/path/name/sginr.so'
	IDL> RESULT=CALL_EXTERNAL(so,'sgimain',nr_name,iarg,rarg,darg)
	where
...
int cspfourn(int *iarg, float *rarg);
int cfour1(int *iarg, float *rarg);
int cfit_(int *iarg, float *rarg);
int cfitexy_(int *iarg, float *rarg);
int crlft3(int *iarg, float *rarg);
void fourn_();
void spfourn_();
void four1_();
void fit_();
