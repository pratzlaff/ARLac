...
;		* may be a float or string array
;		* if float, assumed to be decimal degrees unless
...
if szdec ne 7 then begin			;(all numbers
  ...
  for i=0L,ndec-1L do begin	;{for each entry
    ...
    if keyword_set(dsep) then begin
      ...
      dd=abs(long(cc[0])) & dm=0. & ds=0.
      if ncc gt 1 then dm=long(cc[1])
      if ncc gt 2 then ds=float(cc[2])
      xdec[i]=sd*(dd+(dm+(ds/60.))/60.)
    endif else begin
      xdec[i]=float(dec[i])
    endelse
  endfor			;I=0,NDEC-1}
...
rrform='f6.2' & rhform='i2.2' & rmform='i2.2' & rsform='f6.2'
ddform='f6.2' & dmform='i2.2' & dsform='f6.2'
...
;	fancier output
if runit eq 'hms' then begin
  rh=fix(xra) & rm=fix((xra-rh)*60.) & rs=float( (xra-rh-(rm/60.))*3600. )
...
if dunit eq 'dms' then begin
  xdec=abs(xdec) & dd=fix(xdec) & dm=fix((xdec-dd)*60.) & ds=float( (xdec-dd-(dm/60.))*3600. )
