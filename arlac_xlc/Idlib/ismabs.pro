...
if keyword_set(tb) then sig0 = 1e-22 else sig0 = 1e-24
...
sigma = 0.*kev & sz = size(kev) & list = sz(0)
if list gt 1 then begin
  print, "sorry, don't know how to handle structures larger than 1D array"
  return,0.*kev
endif

if list eq 0 then begin
...
if list eq 1 then begin
  if keyword_set(tb) then begin
    h1 = where(kev ge 0.0124 and kev lt 0.53) 
    h2 = where(kev ge 0.53 and kev lt 5.) 
    if h1(0) ne -1 then sigma(h1) = 0.65*sig0*kev(h1)^(-3)
    if h2(0) ne -1 then sigma(h2) = 2.00*sig0*kev(h2)^(-2.5)
  endif else begin
    for i=0,14-1 do begin
      h1 = where(kev ge el(i) and kev lt eh(i))
      if h1(0) ne -1 then sigma(h1)=c0(i)+c1(i)*kev(h1)+c2(i)*kev(h1)^2
    endfor
    sigma = sig0*sigma*kev^(-3)
  endelse
endif
...
if keyword_set(fh2) then begin
  if fh2 lt 0. then fh2 = 0.26
  if list eq 0 then begin
    if kev lt 1. then sigmah2 = 0.16 * fh2 * kev^(-3.54)
  endif
  if list eq 1 then begin
    h1 = where(kev lt 1.)
    if h1(0) ne -1 then sigmah2(h1) = 0.16 * fh2 * kev(h1)^(-3.54)
  endif
endif
